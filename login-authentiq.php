...
$_SESSION['WPOA']['PROVIDER'] = 'Authentiq';
define('HTTP_UTIL', get_option('wpoa_http_util'));
define('CLIENT_ENABLED', get_option('wpoa_authentiq_api_enabled'));
define('CLIENT_ID', get_option('wpoa_authentiq_api_id'));
define('CLIENT_SECRET', get_option('wpoa_authentiq_api_secret'));
define('REDIRECT_URI', rtrim(site_url(), '/') . '/');
define('URL_AUTH', "https://connect.authentiq.io/authorize?");
define('URL_TOKEN', "https://connect.authentiq.io/token?");
define('URL_USER', "https://connect.authentiq.io/userinfo?");
...
function get_oauth_token($wpoa) {
	...
	// parse the result:
  $result_obj = json_decode($result, true); // PROVIDER SPECIFIC: Authentiq encodes the access token result as json by default

	$access_token = $result_obj['access_token']; // PROVIDER SPECIFIC: this is how Authentiq returns the access token KEEP THIS PROTECTED!
	$expires_in = $result_obj['expires_in']; // PROVIDER SPECIFIC: this is how Authentiq returns the access token's expiration
...
function get_oauth_identity($wpoa) {
	...
	$params = array(
		'access_token' => $_SESSION['WPOA']['ACCESS_TOKEN'], // PROVIDER SPECIFIC: the access token is passed to Authentiq using this key name
	);
	$url_params = http_build_query($params);
	...
	switch (strtolower(HTTP_UTIL)) {
		case 'curl':
			...
			$curl = curl_init();
			curl_setopt($curl, CURLOPT_URL, $url);
			// PROVIDER NORMALIZATION: Authentiq requires that we send the access token via a bearer header...
			// curl_setopt($curl, CURLOPT_HTTPHEADER, array("Authorization: Bearer " . $_SESSION['WPOA']['ACCESS_TOKEN']));
			curl_setopt($curl, CURLOPT_RETURNTRANSFER, 1);
			$result = curl_exec($curl);
	...
	$oauth_identity['id'] = $result_obj['sub']; // PROVIDER SPECIFIC: this is how Authentiq returns the user's unique id
  if(isset($result_obj['given_name'])){
    //PROVIDER SPECIFIC: this is how Authentiq returns the name
    $oauth_identity['username'] = $result_obj['given_name'];
    $oauth_identity['nickname'] = $result_obj['given_name'];
    $oauth_identity['first_name'] = $result_obj['given_name'];
  }
  if(isset($result_obj['family_name'])){
    //PROVIDER SPECIFIC: this is how Authentiq returns the family name
    $oauth_identity['last_name'] = $result_obj['family_name'];
  }
  if(isset($result_obj['email'])){
    //PROVIDER SPECIFIC: this is how Authentiq returns the email
